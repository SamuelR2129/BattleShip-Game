const computerAI = (player) => {
  ...
  playerBoard.opponentBoard().forEach((location, index) => {
    // get all available shots
    if (location === "empty") {
      availableShots.push(index);
    } else if (location === "hit") {
  ...
  // remove doubles
  detectedShip = detectedShip.filter((cell, index, thisArray) => {
    return thisArray.lastIndexOf(cell) === index;
  });
